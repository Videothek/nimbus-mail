...
- **Maintain `SBOM.md` on every dependency change.** Adding, removing, or upgrading a package in any `Cargo.toml` (workspace or per-crate) or in `ui/package.json` requires a corresponding edit to `SBOM.md`: list the package, its licence, what category that licence falls into (permissive / weak copyleft / strong copyleft), and update the "Last manual reconciliation" date at the bottom of the inventory section. Introducing a stronger copyleft licence than what's already in the tree (e.g. AGPL-3.0 when we currently top out at GPL-3.0) is a project-level decision — surface it explicitly to Nick / Jannik before merging, don't slip it into a routine PR. The licence cheat-sheet at the top of `SBOM.md` is the source of truth on what each licence forces our distribution model to look like.
